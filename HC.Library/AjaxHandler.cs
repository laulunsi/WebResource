...
namespace HC.Ajax
{
    ...
    public class AjaxHandler : IHttpHandler, IRequiresSessionState
    {
        protected static string BasePath =
            VirtualPathUtility.AppendTrailingSlash(HttpContext.Current.Request.ApplicationPath);
        ...
        ///     实现IHttpHandler接口
        /// </summary>
        /// <param name="context">当前上下文环境，当前HttpContext.Current</param>
        public void ProcessRequest(HttpContext context)
        {
            ...
            catch (XmlException ex)
            {
                ...
                notefilepath = HttpContext.Current.Server.MapPath(notefilepath);
        ...
        internal static Dictionary<string, string> GetResult(string className, string methodName, XmlDocument requestXml)
        {
            if (Hash.ContainsKey(className))
            {
                ...
                if (type != null)
                {
                    var hande = Activator.CreateInstance(type) as AjaxHandler;
            ...
            {
                if (assembly.FullName.Contains("HC.Ajax"))
                {
                    Type[] classes = assembly.GetTypes();
                    foreach (Type type in classes)
                    {
                        if (IsInherit(type, typeof (AjaxHandler)) &&
        ...
        public static string GetNodeInnerText(XmlDocument xmldoc, string nodeName)
        {
            ...
            if (xmldoc.DocumentElement != null)
            {
                ...
                {
                    innerText = xmlNode.InnerText.Trim();
                }
            }
